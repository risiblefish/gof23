 ...
 * 问： 如果想让代理类 不止代理Movable, 而是可以代理任何接口，比如计算时间，记录日志等行为，具有普遍性，该怎么做？
 * <p>
 * v07: 本质上讲，这种是分离代理行为和被代理对象。可以用jdk的动态代理来实现。
 * <p>
 * 对比一下静态代理，因为知道被代理对象实现了哪个接口，（即知道了这个接口里有哪些方法），所以静态代理可以代理到具体的方法上去。
 * 试想，万一被代理的类没有实现接口，这种情况不能使用静态代理，该如何解决呢
 */
public class Tank07 {
    public static void main(String[] args) throws InterruptedException {
        Tank01 tank = new Tank01();
        Movable m = (Movable) Proxy.newProxyInstance(Tank01.class.getClassLoader(),
                new Class[]{Movable.class}, new TankTimeProxy(tank));
        m.move();
    }
}
...
class TankTimeProxy implements InvocationHandler {
    ...
    public TankTimeProxy(Tank01 tank) {
        this.tank = tank;
    }
